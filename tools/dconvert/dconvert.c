...
int ConvertAndWrite(){
	...
	while(i < inSize){
		...
		if(transformType == 0){//raw 1:1 write
			...
			outBuf[outSize++] = inBuf[i++];
		}else if(transformType == 2){//4bit normalized DPCM
		/*	outSize++;
			i++;
			if(++j == 16){//normalization value
				accum = inBuf[i++];
				outBuf[outSize++] = accum;	
			}else{//normal 4bit sample
				unsigned char t = (inBuf[i]>>4)&0xFF;
				accum += (inBuf[i]>>4);
			}
		*/
		}
